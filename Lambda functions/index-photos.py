...
def lambda_handler(event, context):
    ...
    for record in event['Records']:
        ...
        response = client.head_object(Bucket=bucket,Key=photokey)
        print(response)
        print(response['ResponseMetadata']['HTTPHeaders']['content-type'])
        customlabels=[]
        if  len(response['ResponseMetadata']['HTTPHeaders']['content-type'].split(';'))>2:
            customlabels=response['ResponseMetadata']['HTTPHeaders']['content-type'].split(';')[2].split(',')
        print(customlabels)
